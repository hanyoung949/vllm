...
def test_validate_cycles_pass_and_fail():
    ...
    with pytest.raises(ValueError, match="cycle desync"):
...
def test_advance_untracked_fails():
    tracker = SplitDVIStateTracker()
    with pytest.raises(ValueError, match="untracked request"):
...
def test_generation_epoch_from_scheduler():
    ...
    with pytest.raises(ValueError, match="generation desync"):
